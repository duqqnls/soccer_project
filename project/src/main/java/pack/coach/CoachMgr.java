...
public class CoachMgr {
    ...
    public ArrayList<CoachBean> getCoachesByCategory(String category) {
        ArrayList<CoachBean> list = new ArrayList<>();
        try {
            conn = ds.getConnection();
            String sql = "SELECT * FROM coach WHERE coach_pos=?";
    ...
    public ArrayList<CoachBean> getAllCoaches() {
        ArrayList<CoachBean> list = new ArrayList<>();
        try {
            conn = ds.getConnection();
            String sql = "SELECT * FROM coach WHERE coach_pos IN ('수석코치', '코치', 'GK코치', '피지컬코치', '비디오코치')";
            pstmt = conn.prepareStatement(sql);
            rs = pstmt.executeQuery();
    ...
    public boolean isCoachNameDuplicate(String coachName) {
        boolean isDuplicate = false;
        try {
            conn = ds.getConnection();
            String checkSql = "SELECT COUNT(*) FROM coach WHERE coach_name = ?";
    ...
    public CoachBean getByCoachCode(int code) {
        CoachBean coach = null;
        try {
            conn = ds.getConnection();
            String sql = "SELECT * FROM coach WHERE coach_code = ?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, code);
    ...
    public boolean isCodeDuplicate(int code) {
        boolean isDuplicate = false;
        try {
            conn = ds.getConnection();
            String checkSql = "SELECT COUNT(*) FROM coach WHERE coach_code = ?";
            pstmt = conn.prepareStatement(checkSql);
            pstmt.setInt(1, code);
            rs = pstmt.executeQuery();
            if (rs.next() && rs.getInt(1) > 0) {
